...
function Page() {
  ...
  if (false)
    console.log(selected, viewDialog, addDialog, editDialog, deleteDialog);
  ...
  const promotionsActive = promotions.filter(
    (p) => !isExpired(p.expireAt) && p.status === "ACTIVE"
  ).length;

  const conversionRate =
    promotions
      .filter((x) => !!x.maxUses)
      .reduce((sum, p) => sum + (p.maxUses ?? 0), 0) -
    promotions
      .filter((x) => !!x.maxUses)
      .reduce((sum, p) => sum + p.usedCount, 0);
    ...
    },
    {
      ...
      value: promotions.reduce(
        (total, p) =>
          total +
          (p.percentage > 0
            ? p.amount * p.usedCount
            : (p.percentage * p.usedCount) / 100),
        0
      ),
      isMoney: true,
      ...
      value:
        conversionRate > 0
          ? conversionRate /
            promotions
              .filter((x) => !!x.maxUses)
              .reduce((total, p) => total + (p.maxUses ?? 0), 0)
          : 0,
      valueName: "%",
  ...
  return (
    <PageLayout
      isLoading={getPromotions.isLoading || getStocks.isLoading}
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex gap-4 flex-wrap">
            ...
            <Button>
              <CirclePlus size={16} onClick={() => setAddDialog(true)} />
              {"Créer une promotion"}
            </Button>
          </div>
        </CardContent>
                  ...
                  <TableRow key={promotion.id}>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        <p>
                          {format(promotion.expireAt, "dd/MM/YYY", {
                            locale: fr,
                          })}
                        </p>
                      </div>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          isExpired(promotion.expireAt)
                            ? "destructive"
                            : "outline"
                        }
                      >
                        {isExpired(promotion.expireAt) ? "Expiré" : "Actif"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      {promotion.stock.length > 0
                        ? promotion.stock.map((x, id) => (
                            <div key={id} className="mt-1">
                              {stocks.find((y) => x === y.id)?.productVariant
                                ?.name ?? "N/A"}
                            </div>
                          ))
                        : "N/A"}
